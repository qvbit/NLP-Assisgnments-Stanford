...
def minibatch_parse(sentences, model, batch_size):
    ...
    partial_parses = [PartialParse(s) for s in sentences] # Create a PartialParse object for each sentence in sentences.
    unfinished_parses = partial_parses # Shallow copy
    
    while len(unfinished_parses) > 0:
        minibatch = unfinished_parses[:batch_size]
        
        # We apply all tranitions to the minibatch before moving onto the next one
    ...
    # All sentences have been parsed so we can now extract the dependency list from each of them.
