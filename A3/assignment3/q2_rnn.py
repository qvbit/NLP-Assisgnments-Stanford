...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        len_add = max_length - len_sent
        if len_add > 0:
            sentence = sentence + [zero_vector]*len_add
            labels = labels + [zero_label]*len_add
            mark = [True] * len_sent + [False]*len_add
        else:
            mark = [True] * max_length
            sentence = sentence[:max_length]
            labels = labels[:max_length]
        ret.append((sentence, labels, mark))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits = masked_logits,
