...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        grads_varz = optimizer.compute_gradients(loss)
        varz = [out[1] for out in grads_varz]
        grads = [out[0] for out in grads_varz]
        if self.config.clip_gradients:
            grads = tf.clip_by_global_norm(grads, clip_norm=self.config.max_grad_norm)[0]
        grads_varz = list(zip(grads, varz))
        self.grad_norm = tf.global_norm(grads)
  
        train_op = optimizer.apply_gradients(grads_varz)
        ### END YOUR CODE
