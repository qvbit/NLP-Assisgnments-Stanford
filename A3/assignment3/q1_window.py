...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        T = len(sentence)
        sentence = [start]*window_size + sentence + [end]*window_size # pad the edges of the sentence with start and end token(s)
        features = [(list(flatten(sentence[i:i+2*window_size+1])), labels[i]) for i in range(T)]
        windowed_data += features
    ### END YOUR CODE
