...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_z = tf.get_variable('W_z', shape=(self.state_size, self.state_size), initializer=init, dtype = tf.float32)
            U_z = tf.get_variable('U_z', shape=(self.input_size, self.state_size), initializer=init, dtype = tf.float32)
            b_z = tf.get_variable('b_z', shape=(self.state_size,), initializer=tf.constant_initializer(0), dtype = tf.float32)
            W_r = tf.get_variable('W_r', shape=(self.state_size, self.state_size), initializer=init, dtype = tf.float32)
            U_r = tf.get_variable('U_r', shape=(self.input_size, self.state_size), initializer=init, dtype = tf.float32)
            b_r = tf.get_variable('b_r', shape=(self.state_size,), initializer=tf.constant_initializer(0), dtype = tf.float32)
            W_o = tf.get_variable('W_o', shape=(self.state_size, self.state_size), initializer=init, dtype = tf.float32)
            U_o = tf.get_variable('U_o', shape=(self.input_size, self.state_size), initializer=init, dtype = tf.float32)
            b_o = tf.get_variable('b_o', shape=(self.state_size,), initializer=tf.constant_initializer(0), dtype = tf.float32)
